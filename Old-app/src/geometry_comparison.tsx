...
function genCurve(n_eq: number, tau: number, p: number, times: number[]): CurvePoint[] {
  return times.map((t) => ({
    t,
...
const MIN_T = 0;
const LOG_TIMES: number[] = (() => {
  const pts: number[] = [MIN_T];
  for (let e = 0; e <= 5.3; e += 0.06) pts.push(Math.pow(10, e));
  return pts;
})();

const STUB_FAMILIES: StubFamily[] = [
...
function buildStubDataset(): DataRow[] {
  const rows: DataRow[] = [];
...
const PRESET_TIMES: number[] = [1000, 15000, 60000];

function formatMol(v: number | null | undefined): string {
...
function interpAt(points: DataRow[], tQuery: number): number | null {
...
export default function GeometryComparisonTool(): React.ReactElement {
  const [rawRows, setRawRows] = useState<DataRow[] | null>(null); // null => use stub
  const [tProcess, setTProcess] = useState<number>(15000);
  const [logSlider, setLogSlider] = useState<number>(Math.log10(15000));
  const [fileName, setFileName] = useState<string | null>(null);
  const [parseError, setParseError] = useState<string | null>(null);
  const [hidden, setHidden] = useState<Record<string, boolean>>({});

  const isStub = rawRows === null;
  const rows: DataRow[] = rawRows || buildStubDataset();

  const geometries: string[] = useMemo(() => {
    const seen: string[] = [];
    rows.forEach((r) => { if (!seen.includes(r.geometry)) seen.push(r.geometry); });
    return seen;
  }, [rows]);

  const seriesByGeom: Record<string, DataRow[]> = useMemo(() => {
  ...
  const presetRankings: PresetRanking[] = useMemo(() => {
  ...
  const rankingStable: boolean = useMemo(() => {
    ...
    Papa.parse<Record<string, unknown>>(file, {
      ...
      complete: (res: ParseResult<Record<string, unknown>>) => {
        try {
          ...
          const cols = fields.map((f: string) => f.trim().toLowerCase());
          const geomKey = fields[cols.indexOf("geometry")];
          const tKey = fields[cols.indexOf("t")];
          const nKey = fields[cols.indexOf("n")];
          if (!geomKey || !tKey || !nKey) {
            setParseError("CSV needs columns: geometry, t, n (case-insensitive).");
            return;
          }
          const parsed: DataRow[] = res.data
            .filter((r: Record<string, unknown>) => r[geomKey] !== undefined && r[tKey] !== undefined && r[nKey] !== undefined)
            .map((r: Record<string, unknown>) => ({
              geometry: String(r[geomKey]),
              t: Number(r[tKey]),
              n: Number(r[nKey]),
            }));
          if (!parsed.length) {
  ...
  const toggleHidden = (g: string): void => {
    setHidden((h) => ({ ...h, [g]: !h[g] }));
  };
                ...
                const numericValue = Array.isArray(value)
                  ...
                  : typeof value === "number"
                    ...
                    : value == null || value === ""
                      ? null
                      : Number(value);
                return [formatMol(numericValue), `${name ?? ""}`];
              }}
              labelFormatter={(label: any) => `t = ${formatTime(typeof label === "number" ? label : Number(label))}`}
              contentStyle={{ fontFamily: "IBM Plex Mono", fontSize: 12, border: `1px solid ${COLORS.grid}`, borderRadius: 4 }}
            />
            <Legend
              onClick={(entry: LegendPayload): void => {
                if (typeof entry.dataKey === "string") {
                  toggleHidden(entry.dataKey);
                }
              }}
